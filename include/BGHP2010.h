...
void findKeyEdges( KeyEdgesMap &KeyEdges, Triangulation &D, const PointContainer &P )
{
         ...
         vit!=D.finite_vertices_end(); ++vit )
    {
        ...
        for( size_t cone=1; cone<6; cone+=2 )
        {
            ...
            if( fan.size() > 0 )
            {
                KeyEdges[FIRST][w][flattenedCone] = D.source( fan.front() );
                KeyEdges [LAST][w][flattenedCone] = D.source( fan.back() );
            }
        }
    }
}
                    ...
                    KeyEdgesContainer &KeyEdges )
{
    ...
    if( v == SIZE_T_MAX )
        return false;
    // get cone (v,w)
    size_t w_cone = getCone(w,v,P) / 2;

    if( w_cone != i )
        return false;

    size_t iPlusOne = ((i+1) % 3),
           iLessOne = ((i-1+3) % 3);

    return ( w == KeyEdges[FIRST][u][iLessOne] && w != KeyEdges[CLOSEST][u][iLessOne] )
        || ( w == KeyEdges [LAST][u][iPlusOne] && w != KeyEdges[CLOSEST][u][iPlusOne] );
}
...

    {
        ...
        for( size_t u=0; u<n; ++u ) {
                      ...
                      ++it )
            {
                auto e = *it;
                auto v = D.target(e);
                size_t i = getCone(u,v,P) / 2;
                size_t iPlusOne = (i + 1) % 3;
                size_t iLessOne = (i - 1 + 3) % 3;
                auto w = KeyEdges[FIRST][u][iLessOne];

                if( w != SIZE_T_MAX && is_i_relevant( w, u, i, D, P, KeyEdges ) ) {
                    E.emplace_back( w, u );
        ...
        for( size_t u=0; u<n; ++u ) {
                      ...
                      ++it )
            {
                ...
                size_t iPlusOne = (i + 1) % 3;
                size_t iLessOne = (i - 1 + 3) % 3;
                auto w = KeyEdges[LAST][u][iPlusOne];
        ...
        for( auto e : E )
        {
            size_t cone = getCone(e.first,e.second,P);
            auto pair1 = make_pair(e.first,cone),
                 pair2 = make_pair(e.second,(cone+3)%6);
