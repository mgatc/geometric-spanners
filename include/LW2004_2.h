...
class GraphPrinter {
    ...
    unordered_set<unsignedPair,boost::hash<unsignedPair>> E;
    vector<Point> points;

    public:
            GraphPrinter(vector<Point> points, unordered_set<unsignedPair,boost::hash<unsignedPair>> E, double radiusOfPoints = 0.09) {
            this-> E = E;
        ...
        void draw(string fName = "temp") {
            ...
            for(pair<unsigned,unsigned> p : E) {
            ...
            for(unsigned i = 0; i < points.size(); i++)
                fprintf(fp,"\\draw [fill=red,stroke=red] (%f,%f) circle [radius=%f] node[label=above:%u] {};\n",points[i].x(),points[i].y(),radiusOfPoints,i);
...
void generateRandomPoints(vector<Point> &points, const unsigned n, const string outputFileName = "") {
...
struct comparatorForMinHeap {
    bool operator()(const unsignedPair &n1, const unsignedPair &n2) const {
        return (n1.first > n2.first) || ((n1.first == n2.first) && (n1.second > n2.second));
    }
};

typedef boost::heap::fibonacci_heap<unsignedPair,boost::heap::compare<comparatorForMinHeap>> Heap;
...
inline void createNewEdge(unordered_set<unsignedPair,boost::hash <std::pair<unsigned, unsigned>>> &E, const unsigned i, const unsigned j, const unsigned n) {
    if( std::max(i,j) > n-1)
...
void LW2004_2( RandomAccessIterator pointsBegin, RandomAccessIterator pointsEnd, OutputIterator result ){
    ...
    vector<unsigned> piIndexedByV(n), piIndexedByPiU(n);
    vector<unordered_set<unsigned>> currentNeighbours(n);
   ...
   // unsigned maxDegree = 0;
    ...
    unsigned i = 1;
    while(!H.empty()) {
        unsignedPair p = H.top();
        H.pop();

        for(unsigned neighbour : currentNeighbours[p.second]) {
            if(!currentNeighbours[neighbour].empty()) {
                auto ptr = currentNeighbours[neighbour].find(p.second);
                currentNeighbours[neighbour].erase(ptr);
            }
            unsignedPair q = make_pair((*handleToHeap[neighbour]).first-1,neighbour);
...
//    unordered_set<unsignedPair,boost::hash <std::pair<unsigned, unsigned>>> ePrime; // without set duplicate edges could be inserted (use the example down below)
//    vector<bool> isProcessed(n, false);
//
//    for(unsigned u : piIndexedByPiU) {
//        //cout << "\n" << u <<": \n===================\n";
//
...
//            unsigned closestToUInTheCurrentCone; // stores the ID of the point that achieves closestDistanceSquared
//            unsigned prev; // stores the prev point in the cone
