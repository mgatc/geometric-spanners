...
//        }
        ...
        tuple<index_t,index_t,index_t> getNeighborsInCone(const index_t p, const index_t q, const cone_t cone, const cone_t numCones,
            ...
            return make_tuple(j,i,k);
        }
        void addEasyEdges(const index_t j, const index_t i, const index_t k,
                          const vector<index_t>& Q, vector<Edge>& add,
                          const unordered_set<index_t>& Forbidden = {}) {
 ...
 */
        ...
        inline void wedge<7>(const DelaunayTriangulation &DT,
            ...
            index_t j,i,k;
            vector<index_t> Q;
            tie(j,i,k) = getNeighborsInCone(p,q,cone,numCones,DT,handles,closest,Q);
...
//                }
            ...
            copy(add.begin(),add.end(),back_inserter(addToE_star));
        }
...
//            auto firstInQPrimeIterator = ;
            ...
            if( i > firstInQPrime ) {
                ...
                swap(iLessOneCcw,iPlusOneCcw);
                swap(iLessOneCw, iPlusOneCw );
                //swap(j,k);
                swap(firstInQPrime,lastInQPrime);
            }
